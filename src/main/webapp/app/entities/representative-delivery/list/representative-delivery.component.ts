...
@Component({
  selector: 'jhi-representative-delivery',
  templateUrl: './representative-delivery.component.html',
})
export class RepresentativeDeliveryComponent implements OnInit {
  representativeDeliveries?: IRepresentativeDelivery[];
  isLoading = false;
  totalItems = 0;
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.representativeDeliveryService
      .query({
        page: pageToLoad - 1,
      ...
      .subscribe({
        next: (res: HttpResponse<IRepresentativeDelivery[]>) => {
  ...
  trackId(_index: number, item: IRepresentativeDelivery): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: IRepresentativeDelivery[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/representative-delivery'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
    ...
    this.representativeDeliveries = data ?? [];
