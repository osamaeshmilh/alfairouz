...
@Component({
  selector: 'jhi-specimen-type',
  templateUrl: './specimen-type.component.html',
})
export class SpecimenTypeComponent implements OnInit {
  specimenTypes?: ISpecimenType[];
  isLoading = false;
  totalItems = 0;
  ...
  trackId(_index: number, item: ISpecimenType): number {
    return item.id!;
  }
  ...
  protected onSuccess(data: ISpecimenType[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.router.navigate(['/specimen-type'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
